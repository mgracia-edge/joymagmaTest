...
exports.bind = function () {
    ...
    let BannerVODSchema = new Schema({
        "name": {type: String, index: true},
        "internalName": {type: String, index: true},
        "poster" : { type: [posterSchema], required: false },
        "defaultPoster": {type: String},
        "useDefault" : Boolean
    });

    db.BannerVOD = mongoose.model('bannersvods', BannerVODSchema);

    
    /*  eventSport  */
    let deportNameSchema = new Schema({
    ...
    let EventSportSchema = new Schema({
        "deportGroupName" : {type: String },
        "deportName" : [deportNameSchema]
        
    })
    db.EventSport = mongoose.model('eventsports', EventSportSchema)

    // Radios
    let radiosSchema = new Schema({
        "format": String,
        "name": String,
        "url": String,
        "logo": String,
    ...
    db.Radios = mongoose.model('radios', radiosSchema)
...
//     deportGruup
//     deportName: [{ deportName, eventName, enabled, addInfo }]
// ]
